...
{   
  ...
  dStart = RTC.now();
  dDummy =  dStart.unixtime();
  dDummy =  (long) ((dDummy+5)/60)+1;
  dDummy = dDummy*60;
  dStart = dDummy;
  seqLength = 15;
  seqPeriod = 3600;
  ...
  showString(PSTR("Node V3_4_RecordOnly\r\nCO2 Sensor Details\r\n"));
  ...
  initAlarm(); // Initialise RTC alarm
...
void initAlarm(void)
{
  ...
  Wire.write(0x0E);
  Wire.write(0b00000101); // Enables interrupt on alarm1
  Wire.endTransmission();
  
  Wire.beginTransmission(0x68);
  Wire.write(0x07);  
  Wire.write(decToBcdAon(dStart.second()));
  Wire.write(decToBcdAon(dStart.minute()));
  Wire.write(decToBcdAon(dStart.hour()));
  Wire.write(decToBcdAoff(0));
  Wire.endTransmission();
...
void setAlarm(void)
{
  ...
  Wire.beginTransmission(0x68);
  Wire.write(0x07);  
  Wire.write(decToBcdAon(dAlarm.second()));
  Wire.write(decToBcdAon(dAlarm.minute()));
  Wire.write(decToBcdAon(dAlarm.hour()));
  Wire.endTransmission();
}
...
void clearAlarm(void)
{
  ...
  while(Wire.available()) {alarmFlag = Wire.read();}
  
  alarmFlag = alarmFlag & 0b11111100;
  
  Wire.beginTransmission(0x68);
  Wire.write(0x0F);
  Wire.write(alarmFlag);
  Wire.endTransmission();
}  

// Conversion function to Binary Coded Decimal for setting the alarm time 
byte decToBcdAoff(byte val) {return ( 128 + (val/10*16) + (val%10) );}
byte decToBcdAon(byte val) {return ( (val/10*16) + (val%10) );}
